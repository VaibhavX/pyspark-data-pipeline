...
# key created in the first resource in this file
...
resource "aws_instance" "bastion_host" {
  ...
  user_data                   = <<-EOF
    ...
    aws s3 cp s3://<SOURCE_LOCATION>/sql/copy_data.sql /home/ec2-user/sql/copy_data.sql
    aws s3 cp s3://<SOURCE_LOCATION>/sql/ratings_table_ddl.sql /home/ec2-user/sql/ratings_table_ddl.sql
    mkdir -p /home/ec2-user/data
    aws s3 cp s3://<SOURCE_LOCATION>/data/ratings_ml_training_dataset.csv /home/ec2-user/data/ratings_ml_training_dataset.csv
    EOF
  user_data_replace_on_change = true

  subnet_id                   = data.aws_subnet.public_subnet.id # Use the public subnet from network.tf file
  vpc_security_group_ids      = [aws_security_group.bastion_host.id]      # Use the security group created for the bastion host in the network.tf file
